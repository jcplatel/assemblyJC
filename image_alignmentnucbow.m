...
%smooth image before ???
% blue=imread([path,'blue2.tif']);
% blue=blue(1:512,1:512);
% red=imread([path,'red2.tif']);
% red=red(1:512,1:512);
red890=imread([path,'red890.tif']);
red1040=imread([path,'red1040.tif']);
red1100=imread([path,'red1100.tif']);
% green=imread([path,'green2.tif']);
% green=green(1:512,1:512);
% calcium=imread([path,'calcium2.tif']);
...
%% 
reg_obj = imregcorr(red, calcium,'similarity');
% reg_obj = imregcorr(green, calcium);
T = reg_obj.T;

aligned_red = imwarp(red, affine2d(T), 'OutputView', imref2d(size(red)));
imwrite(aligned_red, ([path,'aligned_red.tif']), 'tif');

reg_obj = imregcorr(green, calcium,'similarity');
% reg_obj = imregcorr(green, calcium);
T = reg_obj.T;
...
reg_obj = imregcorr(blue, calcium,'similarity');
% reg_obj = imregcorr(green, calcium);
T = reg_obj.T;
...
composite(:,:,1)=aligned_red;
composite(:,:,2)=aligned_green;
composite(:,:,3)=aligned_blue;
%composite(:,:,4)=calcium;
imwrite(composite, ([path,'composite.tif']), 'tif');

imwrite(aligned_red, ([path,'compositecalcium.tif']), 'tif', 'WriteMode', 'overwrite');

function [DataOut, Stats, options] = preprocessing_5(F, speed, corrXY, colorcell,options)


%[Tr1b,Raster, Race,RasterRace, NCell,SumAct,MAct, start_PC,nPC_Final,Race_For_Clustering,options]=preprocessing_5(Tr1b,speed,corrXY)
%% Load settings
...
Tr1b = sgolayfilt(Tr1b_clean', 3, options.SG_window)';
...
for i = 1:NCell
    ...
    th(i) = options.threshold_peak * mad_trace * 1.4826;
    
    % Détecter les pics sur la trace complète
    [amplitude, locs] = findpeaks(Tr1b(i, :), 'MinPeakProminence', th(i), 'MinPeakDistance', options.MinPeakDistance);
...
SumAct = sum(Raster, 1);
[pks, locs] = findpeaks(SumAct, 'MinPeakHeight', options.sce_n_cells_threshold, 'MinPeakDistance', options.MinPeakDistancesce);
...
SumAct = sum(Raster, 1);
[pks, TRace] = findpeaks(SumAct, 'MinPeakHeight', options.sce_n_cells_threshold, 'MinPeakDistance', options.MinPeakDistancesce);

NRace = length(TRace);
fprintf('nSCE après filtrage bad_frames : %d\n', NRace);

% ===== CRÉER RACE MATRIX (Participation des cellules aux SCE) =====
MAct = zeros(1, Nz - options.synchronous_frames);
for i = 1:Nz - options.synchronous_frames
    MAct(i) = sum(max(Raster(:, i:i+options.synchronous_frames), [], 2));
end
